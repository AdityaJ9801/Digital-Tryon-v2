...
def main():
    ...
    print(f"Instantiating U-Nets... ON {DEVICE}")
    unet1 = get_unet_by_name("base")
    unet2 = get_unet_by_name("sr")
    print("\nSummary for U-Net 1 (base):")
    summary(unet1, input_size=(3, *IMAGE_SIZE_BASE))

    print("\nSummary for U-Net 2 (sr):")
    summary(unet2, input_size=(3, *IMAGE_SIZE_SR))

    print(f"U-Net 1 (base) size: {get_model_size_mb(unet1):.2f} MB")
    print(f"U-Net 2 (sr) size: {get_model_size_mb(unet2):.2f} MB")
